...
//$result = run_query();
    ...
    if ($adm['position'] == 'Admin') {
        # code...
        $result = verSolicitudC();
    }elseif ($adm['position'] == 'Monitor') {
        # code...
        $result = verSolicitudCMonitor();
    }else{
        $result = verSolicitudCAuxiliar();
    }
    //$result = verSolicitudC();
?>
<!DOCTYPE html>
       ...
       <?php
        ...
        <div class="asideDerecha">
            <div class="menu-listado">
                <div>
                    <h1>Listado Salas Confirmadas</h1>
                </div>            
            </div>
            <?php 
            	if ($result == false ) {
            		echo "No se encuentran solicitudes Confirmadas";
            	}else{
            ?>
            <table class="table">
                <thead>
                ...
                <tbody>
                    <tr>
                        ...
                        <td class="td-btn">                            
                                <a class="btn" href="cancelm.php?id=<?php echo $user['id']; ?>" role="button">Cancelar</a>     
                        </td>
                    </tr> 
                    <?php 
						}	// Fin while
